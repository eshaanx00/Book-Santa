...
export default class MyDonationScreen extends Component {
    ...
    sendNotification=(bookDetails,requestStatus)=>{
      ...
      .then((snap)=>{
        snap.forEach((doc)=>{
          ...
          db.collection("all_notifications").doc(doc.id).update({"message" : message,"notification_status" : "unread","date" : firebase.firestore.FieldValue.serverTimestamp()})
        })
      })
    }
  ...
  getDonorDetails=(donorId)=>{
    db.collection("users").where("email_id","==",donorId).get().then(snap=>{
      snap.forEach((doc)=>{
        this.setState({
